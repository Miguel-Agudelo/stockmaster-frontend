 ...
 *
 * Colores de marca StockMaster:
 *   Naranja primario : #FF7B00
...
/** Formatea una fecha ISO a DD/MM/AAAA HH:MM para CSV/Excel */
...
/** Extrae el valor de un accessor del objeto, respetando fechas y nulos */
 ...
 * @param {string[]} headers  - Cabeceras de columna visibles.
 * @param {string[]} accessors - Claves del objeto de datos.
 ...
 * @param {Promise<{data: ArrayBuffer}>} apiCall  - Llamada axios con responseType:'blob'
...
export const downloadExcelFromBackend = async (apiCall, fileName) => {
  try {
    ...
    showToast(
      "No se pudo generar el archivo Excel. Intente nuevamente.",
      "error",
    );
  }
};
...
// Colores de marca como arrays RGB para jsPDF
const BRAND = {
  orange: [255, 123, 0], // #FF7B00
  darkOrange: [204, 98, 0], // #CC6200
  lightOrange: [255, 243, 229], // #FFF3E5
  white: [255, 255, 255],
  lightGray: [245, 245, 245], // #F5F5F5
  darkGray: [51, 51, 51], // #333333
  medGray: [120, 120, 120],
  danger: [185, 28, 28], // rojo texto
  lightRed: [254, 226, 226], // fondo peligro
  success: [21, 128, 61], // verde texto
  lightGreen: [220, 252, 231], // fondo éxito
};

/**
 * Exporta datos a PDF con encabezado de marca, metadatos y tabla estilizada.
 *
 * @param {object} config
 * @param {string}   config.title         - Título del reporte.
 * @param {string}   config.subtitle      - Subtítulo / descripción breve.
 * @param {string}   config.meta          - Texto de metadatos (fechas, filtros, etc.).
 * @param {string[]} config.headers       - Cabeceras de columna.
 * @param {string[]} config.accessors     - Claves del objeto de datos.
 * @param {object[]} config.data          - Datos a exportar.
 * @param {string}   config.fileName      - Nombre base del archivo.
 * @param {Function} [config.rowStyleFn]  - (item, rowIndex) => { styles, cellStyles } opcional.
 */
export const exportToPdf = async (config) => {
  ...
  // Rectángulo blanco pequeño como "logo placeholder"
  ...
  autoTable(doc, {
    ...
    // Estilos personalizados por fila (peligro/éxito)
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. FUNCIONES ESPECÍFICAS POR REPORTE (lógica de estilos por reporte)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Exporta el reporte de Stock Bajo a PDF con celdas de stock en rojo.
 */
export const exportLowStockToPdf = (data, fileName = "Reporte_Stock_Bajo") => {
  return exportToPdf({
    ...
    rowStyleFn: (item, colIndex) => {
      // Columnas 3 (Stock Actual) y 4 (Stock Mínimo) → rojo
...
/**
 * Exporta el reporte de Movimientos a PDF.
 */
...
/**
 * Exporta el reporte de Más Vendidos a PDF con columnas de ingresos en verde.
 */
...
/**
 * Exporta el reporte de Trazabilidad de Proveedor a PDF.
 */
