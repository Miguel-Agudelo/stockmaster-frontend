...
const ProfilePage = () => {
    ...
    const handleGoogleLinkResponse = useCallback(async (response) => {
        ...
        try {
            ...
            setGoogleMsg({ type: "success", text: "Cuenta de Google vinculada exitosamente." });
        } catch (err) {
            const serverMsg = err.response?.data?.message || "Error al vincular la cuenta de Google.";
    ...
    const handleUnlinkGoogle = async () => {
        ...
        try {
            ...
            const serverMsg = err.response?.data?.message || "Error al desvincular la cuenta de Google.";
            setGoogleMsg({ type: "error", text: serverMsg });
        } finally {
            setGoogleLoading(false);
        }
    ...
    return (
        <div className="main-content">
            ...
            {/* ── Grid principal: [Google | Contraseña] ──────────────────── */}
            <div className={`profile-main-grid${!googleClientId ? " profile-main-grid--single" : ""}`}>

                {/* Tarjeta: Cuenta de Google */}
                {googleClientId && (
                    ...
                    No vinculado
                        ...
                        <div className="profile-google-body">
                            {googleMsg && (
                                <div className={`profile-alert profile-alert--${googleMsg.type}`} style={{ margin: "0 0 16px 0" }}>
                            ...
                            {!googleLinked ? (
                                ...
                                <div className="profile-google-action">
                                    <p className="profile-google-hint">
                                        Si desvincula su cuenta de Google, solo podrá acceder mediante correo y contraseña.
                                    </p>
                                    <button
                                        type="button"
                                    ...
                                    >
                                        {googleLoading ? "Desvinculando..." : "Desvincular cuenta de Google"}
                                    </button>
                                </div>
                            )}
                        </div>
